...
DEPRECATED: This test file tests the old w2t_bkin.domain module which was removed in Phase 3.
Domain models were relocated as follows:
- Config models → w2t_bkin.config package (see tests/unit/test_config.py)
- Sync models → w2t_bkin.sync.models (AlignmentStats)
- Facemap models → w2t_bkin.facemap.models (FacemapBundle, FacemapROI, FacemapSignal)
- Transcode models → w2t_bkin.transcode.models (TranscodedVideo, TranscodeOptions)
- Session models → DEPRECATED (replaced by pynwb.NWBFile)
- Manifest models → REMOVED (not needed in NWB-first architecture)

All tests in this file are now obsolete.
See individual module tests for current model validation tests.
...
# Skip all tests in this module - domain module removed in Phase 3
pytest.skip("test_domain.py deprecated - domain module removed in Phase 3", allow_module_level=True)
...
@pytest.mark.skip(reason="Trial model deprecated - use ndx-structured-behavior TrialsTable")
class TestTrialModel:
    ...
    def test_Should_CreateTrial_When_ValidDataProvided(self):
        ...
        from w2t_bkin.bpod.models_2 import Trial, TrialOutcome
    ...
    def test_Should_RequireAllFields_When_CreatingTrial(self):
        ...
        from w2t_bkin.bpod.models_2 import Trial

        with pytest.raises(ValidationError):
            Trial(trial_number=1, start_time=0.0)


@pytest.mark.skip(reason="BehavioralEvents deprecated - use ndx-structured-behavior EventsTable")
...
@pytest.mark.skip(reason="TrialSummary model deprecated - use ndx-structured-behavior metadata")
class TestTrialSummaryModel:
    """Test TrialSummary domain model structure (Phase 3 - QC).
    ...
    def test_Should_CreateTrialSummary_When_ValidDataProvided(self):
        ...
        from w2t_bkin.bpod.models_2 import TrialSummary
