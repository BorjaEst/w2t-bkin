...
**Prefect-Native Architecture (v0.0.11)** - Fully implemented:
...
- `server start` / `stop` / `status` / `restart` - Prefect server management

**Design Principles**:

- Thin presentation layer - all business logic in flows/ and operations/
- UI-first workflow - Prefect UI at http://localhost:4200 for batch processing and monitoring
- Rich terminal formatting for better UX
- Entry point: `w2t-bkin` command registered in `pyproject.toml`

### 5. Docker Deployment

**Deployment Architecture**: Prefect Work Pools

Containerized deployment using Prefect Docker work pools:

- **Prefect server**: Runs locally via `w2t-bkin server start`
- **Prefect UI**: Port 4200 for monitoring and triggering flows
- **Automatic deployments**: Flows deployed on server startup via Python API
- **Docker workers**: Execute tasks in isolated Docker containers per flow run
- **User-friendly setup**: `.env` auto-generated by `w2t-bkin data init`

**Why Docker Work Pools?**

1. **Isolation**: Each flow run executes in a fresh container
2. **Reproducibility**: Same Docker image guarantees consistent environment across runs
3. **Flexibility**: Different flows can reference different images if needed
4. **Resource management**: Container limits prevent runaway processes
5. **Scaling**: Start more workers (`w2t-bkin worker start --count N`) to handle concurrent runs

**Development Mode Alternative**:

- Use `w2t-bkin server start --dev` for rapid iteration
- Flows run via Prefect Runner in server process (no workers/containers)
- Fast feedback loop for debugging and feature development

**Scaling Model**:

- Horizontal: Start more workers (`w2t-bkin worker start --count 4`)
- Vertical: Adjust container resources via job variables in deployment
- Concurrency: Configure per-worker limits with `--limit` flag
