...
- **Docker workers**: Execute tasks in isolated containers (configured via .env)
- **User-friendly setup**: `.env` auto-generated by `w2t-bkin data init`

**Why Process Pools?**

1. **Security**: No Docker socket access required (eliminates root-equivalent privileges)
2. **Simplicity**: Flows execute as subprocesses within worker containers (no Docker-in-Docker)
3. **Performance**: No container spin-up overhead per flow run
4. **Debugging**: Clearer logs since flows run in known worker containers
5. **Resource Efficiency**: Predictable resource usage and fast startup times

**Scaling Model**:

- Horizontal: Increase `WORKER_REPLICAS` in .env to add more worker containers
- Vertical: Adjust `WORKER_CPU_LIMIT` and `WORKER_MEMORY_LIMIT` per worker
- Concurrency: Each worker handles one flow run at a time (`--limit 1`)
