...
@flow(
    ...
    description="Discover available sessions from raw data directory",
    log_prints=True,
)
def discover_sessions_flow(
    config_path: str | Path,
    subject_filter: Optional[str] = None,
    session_filter: Optional[str] = None,
) -> List[tuple[str, str]]:
    ...
    This flow scans the raw data directory and returns a list of
    (subject_id, session_id) tuples matching the provided filters.
    Useful for previewing which sessions would be processed by batch_process_flow.

    Args:
        config_path: Path to configuration TOML file
        subject_filter: Subject ID filter pattern (e.g., "subject-*")
        session_filter: Session ID filter pattern (e.g., "session-001")
    ...
    Example:
        >>> sessions = discover_sessions_flow(
        ...     config_path="config.toml",
        ...     subject_filter="subject-*"
        ... )
    ...
    run_logger.info(f"Discovered {len(sessions)} sessions " f"(subject_filter: {subject_filter}, session_filter: {session_filter})")

    for subject_id, session_id in sessions:
        run_logger.info(f"  - {subject_id}/{session_id}")
