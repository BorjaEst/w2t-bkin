...
class SessionFlowConfig(BaseModel):
    ...
    base_config_path: Optional[str] = Field(
        None,
        description="Base configuration file (package defaults). If not provided, uses built-in configs/standard.toml.",
        examples=["/app/configs/standard.toml", "/configs/container.toml"],
    )
    project_config_path: Optional[str] = Field(
        None,
        description="Project-specific configuration file (overrides base). Generated by 'w2t-bkin data init'.",
        examples=["/data/project/config.toml", "/home/user/experiment/config.toml"],
    )
    ...
    @field_validator("base_config_path", "project_config_path")
    @classmethod
    def validate_config_path(cls, v: Optional[str]) -> Optional[str]:
        """Validate config path format."""
        if v is not None and not v.endswith(".toml"):
